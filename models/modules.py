...
class embedding(nn.Module):
    def __init__(self, vocab_size, num_units, zeros_pad=True, scale=True):
        ...
        self.scale = scale
        self.lookup_table = Parameter(torch.rand(vocab_size, num_units))
        nn.init.xavier_normal(self.lookup_table.data)
        if self.zeros_pad:
            self.lookup_table.data[0, :].fill_(0)
    def forward(self, inputs):
        #print ('lookup_table', self.lookup_table)
        if self.zeros_pad:
            self.padding_idx = 0
        else:
            self.padding_idx = -1
...
class new_multihead_attention(nn.Module):

    def __init__(self, num_units, num_heads=8, dropout_rate=0, causality=False, return_att = False):
        '''Applies multihead attention.
        ...
        Args:
            ...
            num_heads: An int. Number of heads.
        '''
        super(new_multihead_attention, self).__init__()
        self.num_units = num_units
        self.num_heads = num_heads
        self.dropout_rate = dropout_rate
        self.causality = causality
        ...
        self.K_proj = nn.Sequential(nn.Linear(self.num_units, self.num_units), nn.ReLU())
        self.V_proj = nn.Sequential(nn.Linear(self.num_units, self.num_units), nn.ReLU())
        #self.G_proj = nn.Sequential(nn.Linear(222, 222), nn.ReLU())
        #self.G_proj = nn.Linear(180, 180)
        self.output_dropout = nn.Dropout(p=self.dropout_rate)

        self.normalization = layer_normalization(self.num_units)

    def forward(self, queries, keys, values, graph):
        # keys, values: same shape of [N, T_k, C_k]
        # queries: A 3d Variable with shape of [N, T_q, C_q]
...
#         print ('outputs', outputs.shape)
        ...
        # Activation
        outputs = F.softmax(outputs, dim=-1)  # (h*N, T_q, T_k)
        #graph_ = self.G_proj(graph.float())
        graph_ = graph
        graph_masks = graph_.repeat(self.num_heads, 1, 1) 
#         print (graph_masks.shape)
#         print (graph_masks, '\n')
        outputs = graph_masks * outputs # (h*N, T_q, T_k)
        outputs = F.normalize(outputs, p=1, dim=-1)
        att_weights = outputs.clone() # (h*N, T_q, T_k)
...
class new_multihead_attention_with_graph_mask(nn.Module):
    ...
    def __init__(self, num_units, num_heads=8, dropout_rate=0, causality=False, return_att = False):
        ...
        Args:
            ...
            num_heads: An int. Number of heads.
        '''
        super(new_multihead_attention_with_graph_mask, self).__init__()
        self.num_units = num_units
        self.num_heads = num_heads
        self.dropout_rate = dropout_rate
        self.causality = causality
    ...
    def forward(self, queries, keys, values, key_mask_ipt, graph = None):
        ...
        att = torch.bmm(Q_, K_.permute(0, 2, 1))  # (h*N, T_q, T_k)
        att = att / (K_.size()[-1] ** 0.5)

        # Key Masking
        key_masks = torch.sign(torch.abs(torch.sum(keys, dim=-1)))  # (N, T_k)
        key_masks = key_masks.repeat(self.num_heads, 1)  # (h*N, T_k)
        key_masks = torch.unsqueeze(key_masks, 1).repeat(1, queries.size()[1], 1)  # (h*N, T_q, T_k)
        padding = Variable(torch.ones(*att.size()).cuda() * (-2 ** 32 + 1))
        condition = key_masks.eq(0.).float()
        att = padding * condition + att * (1. - condition)


        # Causality = Future blinding
        if self.causality:
            diag_vals = torch.ones(*att[0, :, :].size()).cuda()  # (T_q, T_k)
            tril = torch.tril(diag_vals, diagonal=0)  # (T_q, T_k)
            # print(tril)
            masks = Variable(torch.unsqueeze(tril, 0).repeat(att.size()[0], 1, 1))  # (h*N, T_q, T_k)
            padding = Variable(torch.ones(*masks.size()).cuda() * (-2 ** 32 + 1))
            condition = masks.eq(0.).float()
            att = padding * condition + att * (1. - condition)

        # Activation
        att_weights = F.softmax(att, dim=-1)  # (h*N, T_q, T_k)

        graph_masks = graph.repeat(self.num_heads, 1, 1) # (h * N, T_q, T_q) #size torch.Size([512, 73, 73])
        att_weights = graph_masks * att_weights

        att_weights = att_weights / (att_weights.sum(dim = -1, keepdim = True) + 1e-7)

        # Query Masking
        query_masks = torch.sign(torch.abs(torch.sum(queries, dim=-1)))  # (N, T_q)
        query_masks = query_masks.repeat(self.num_heads, 1)  # (h*N, T_q)
        query_masks = torch.unsqueeze(query_masks, 2).repeat(1, 1, keys.size()[1])  # (h*N, T_q, T_k)
        outputs = att_weights * query_masks
...
if __name__ == '__main__':
    att = new_multihead_attention(512)
    att.cuda()
    att2 = new_multihead_attention_with_graph_mask(512)
    att2.cuda()
    att3 = multihead_attention(512)
    att3.cuda()

    ipt1 = torch.randn((4, 1, 512)).cuda()
    ipt2 = torch.randn((4, 15, 512)).cuda()
    graph = (torch.randn((4, 15, 15)) > 0.5).float().cuda()
    mask = (torch.randn((4,15)) > 0.5).float().cuda()
    opt1 = att(ipt1, ipt2, ipt2, graph)
    opt2 = att2(ipt1, ipt2, ipt2, mask, None)
    opt3 = att3(ipt1, ipt2, ipt2)
    pdb.set_trace()
    pass
