...
class GQADataset_super_node(data.Dataset):
    ...
    def convert_graph(self, data_info, bg_class, bbox, gt_graph):
        ...
        macro_rel_loc = {} #Record rel node loaction
        ...
        #Build Empty Macro Rel Node
        for i in range(num_obj):
            for j in range(num_obj):
                if i != j:
                    r_name = '__empty__'
                    pos_rel = len(macro_node)
                    # macro_rel_loc.append(pos_rel)
                    macro_rel_loc[(i,j)] = pos_rel
        ...
        # Build Micro
        micro_rel_loc_pos = 0
        micro_positive_relation = []
        micro_negative_relation = []
        micro_positive_relation_loc = [] #[obj_i, obj_j, rel_category, macro_rel_loc, micro_rel_loc]; mask micro_positive_relation_loc[:,:,2]>0 N,obj_pari,info
        micro_negative_relation_loc = []

        for i in range(num_obj):
            for j in range(num_obj):
                if i != j:
                    nodes_obj_is = micro_positive_node[i]
                    nodes_obj_js = micro_positive_node[j]
                    micro_positive_rel = []
                    micro_positive_idx = []
                    for i_ in range(len(nodes_obj_is)):
                        for j_ in range(len(nodes_obj_js)):
                            ...
                            if key in self.gt_relations:
                                ...
                                r_idx = self.gt_relation_clean.index(self.gt_relations[key])
                                micro_positive_relation_loc.append([i, j, r_idx, macro_rel_loc[(i,j)], micro_rel_loc_pos])
                                micro_positive_rel.append(r_name)
                                micro_positive_idx.append(r_idx)
                                micro_rel_loc_pos += 1
                            else:
                                r_name = PAD
                                r_idx = self.num_relations   #self.num_relations #The last one will be the PAD one
                                micro_positive_relation_loc.append([i, j, r_idx, macro_rel_loc[(i,j)], micro_rel_loc_pos])
                                micro_positive_rel.append(r_name)
                                micro_positive_idx.append(r_idx)
                                micro_rel_loc_pos += 1

                    micro_positive_relation += micro_positive_rel

                    # random sample the rest list for the negative relation (same length as positive rel)
                    gt_relations_neg_idxs = [j for j in range(len(self.gt_relation_clean)) if j not in micro_positive_idx]
                    micro_negative_rel_idxs = random.sample(gt_relations_neg_idxs, len(micro_positive_rel))
                    for r_idx in micro_negative_rel_idxs:
                        micro_negative_relation_loc.append([i, j, r_idx, macro_rel_loc[(i,j)]])
                        if r_idx != len(self.gt_relation_clean):
                            micro_negative_relation.append(self.gt_relation_clean[r_idx])
                        else:
                            micro_negative_relation.append(PAD)

        return macro_node, macro_relation, macro_obj_loc, micro_positive_node, micro_negative_node, \
               micro_positive_relation, micro_negative_relation, micro_positive_relation_loc, micro_negative_relation_loc,
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            with tarfile.open(self.g_tar_fn) as tar_fid:
                ...
                micro_positive_relations, micro_negative_relations, micro_positive_relations_loc, micro_negative_relations_loc\
                    = self.convert_graph(data_info, self.opt.bg_class, bbox, gt_graph)

            macro_nodes_idx = []
            micro_positive_nodes_wrd = []
            micro_negative_nodes_wrd = []
            micro_positive_relations_wrd = []
            micro_negative_relations_wrd = []
            #Macro
            ...
            if len(macro_nodes_idx) + len(qnode_idx) >= self.len_threshold:
                ...
                for node in micro_positive_relations:
                    if node in self.word_converter:
                        node = self.word_converter[node]
                    micro_positive_relations_wrd.append(self.enc_w2id.get(node, UNK))

                for node in micro_negative_relations:
                    if node in self.word_converter:
                        node = self.word_converter[node]
                    micro_negative_relations_wrd.append(self.enc_w2id.get(node, UNK))
                ...
                return vis_fea, np.asarray(macro_nodes_idx).astype('int64'), np.asarray(macro_obj_locs).astype('int64'), \
                       ...
                       np.asarray(micro_positive_relations_wrd).astype('int64'), np.asarray(micro_negative_relations_wrd).astype('int64'), \
                       np.asarray(micro_positive_relations_loc).astype('int64'), np.asarray(micro_negative_relations_loc).astype('int64'), \
...
def collate_fn(data):
    ...
    micro_positive_relations_wrd, micro_negative_relations_wrd, micro_positive_relations_loc, micro_negative_relations_loc, \
    ...
    # max_rel_len
    max_rel_len = 0
    for node in micro_positive_relations_wrd:
        max_rel_len = max(max_rel_len, node.shape[0])

    micro_positive_rel_ipt = np.zeros((batch_size, max_rel_len), dtype='int64')
    micro_negative_rel_ipt = np.zeros((batch_size, max_rel_len), dtype='int64')
    micro_positive_rel_loc = np.zeros((batch_size, max_rel_len, 5), dtype='int64') #[obj_i, obj_j, rel_category, macro_rel_loc, micro_rel_loc_pos];
    micro_negative_rel_loc = np.zeros((batch_size, max_rel_len, 4), dtype='int64') #[obj_i, obj_j, rel_category, macro_rel_loc,];
    micro_positive_rel_ipt[:] = PAD
    micro_negative_rel_ipt[:] = PAD
    micro_positive_rel_loc[:] = LOC_PAD
    micro_negative_rel_loc[:] = LOC_PAD

    for i in range(batch_size):
        if micro_positive_relations_wrd[i].shape[0] != 0:
            micro_positive_rel_ipt[i, :micro_positive_relations_wrd[i].shape[0]] = micro_positive_relations_wrd[i]
            micro_negative_rel_ipt[i, :micro_negative_relations_wrd[i].shape[0]] = micro_negative_relations_wrd[i]
            micro_positive_rel_loc[i, :micro_positive_relations_loc[i].shape[0], :] = micro_positive_relations_loc[i]
            micro_negative_rel_loc[i, :micro_negative_relations_loc[i].shape[0], :] = micro_negative_relations_loc[i]
    ...
    data_ipt['micro_positive_rel_ipt'] = torch.from_numpy(micro_positive_rel_ipt)
    data_ipt['micro_negative_rel_ipt'] = torch.from_numpy(micro_negative_rel_ipt)
    data_ipt['micro_positive_rel_loc'] = torch.from_numpy(micro_positive_rel_loc)
    data_ipt['micro_negative_rel_loc'] = torch.from_numpy(micro_negative_rel_loc)
