...
class AttModel_vis_enc(nn.Module):
    def __init__(self, glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate, maxlen_v, num_classes):
        ...
        '''
        super(AttModel_vis_enc, self).__init__()
        ...
        for i in range(self.num_blocks):
            ...
            self.__setattr__('dec_feed_forward_%d' % i, feedforward(self.hidden_size,
                                                                    [4 * self.hidden_size,
                                                                     self.hidden_size]))
    def forward(self, vis_fea, vis_mask, vis_graph, q_fea, q_graph, q_mask, decMask, visGraph):
        ...
        valid_mask = dec_mask.permute(0, 2, 1)
        graph_cross = 1 - mask
        ...
        if visGraph == False:
            ...
            graph[:, :vis_fea.shape[1], :vis_fea.shape[1]] = vis_graph[:, :vis_fea.shape[1], :vis_fea.shape[1]]
        ...
        return fea_graph, dec_mask, decoder_inputs, valid_mask



class AttModel_vis_dec(nn.Module):
    def __init__(self, glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate, num_classes):
        ...
        Args:
            ...
            dec_voc: vacabulary size of decoder language
        '''
        super(AttModel_vis_dec, self).__init__()
        ...
        for i in range(self.num_blocks):
            ...
            self.__setattr__('dec_feed_forward_%d' % i, feedforward(self.hidden_size,
                                                                    [4 * self.hidden_size,
                                                                     self.hidden_size]))
    def forward(self, fea_graph, dec_mask, decoder_inputs):
        # Decoder
...
class AttModel_syb_enc(nn.Module):
    def __init__(self, glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate, num_classes):
        super(AttModel_syb_enc, self).__init__()
...
class AttModel_syb_dec(nn.Module):
    def __init__(self, glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate, num_classes):
        '''Attention is all you nedd. https://arxiv.org/abs/1706.03762
        Args:
            enc_voc: vocabulary size of encoder language
            dec_voc: vacabulary size of decoder language
        '''
        super(AttModel_syb_dec, self).__init__()
...
class AttModel_MIL_align(nn.Module):
    def __init__(self, hidden_size, num_blocks, num_heads, dropout_rate, maxlen):
    ...
    def forward(self, vis_fea, vis_mask, syb_fea, syb_mask):
        ...
        vis_fea += self.syb_positional_encoding(
            Variable(torch.unsqueeze(torch.arange(0, vis_fea.shape[1]), 0).repeat(vis_fea.shape[0], 1).long().cuda()))
        # att_weights = []
        att_weights = torch.zeros(self.num_blocks, self.num_heads*vis_fea.size(0), vis_fea.size(1), syb_fea.size(1)).cuda()

        for i in range(self.num_blocks):
            vis_fea, att_w = self.__getattr__('enc_self_attention_%d' % i)(vis_fea, syb_fea, syb_fea, vis_mask)
            vis_fea = self.__getattr__('enc_feed_forward_%d' % i)(vis_fea)
            # att_weights.append(att_w)
            att_weights[i] = att_w

        return vis_fea, att_weights
...
class AttModel(nn.Module):
    def __init__(self, glove, hidden_size, num_classes, maxlen_q, maxlen, maxlen_v, num_blocks, num_heads, dropout_rate, dropout_rate_mcb ):
        super(AttModel, self).__init__()
        self.att_vis_enc = AttModel_vis_enc(glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate, maxlen_v, num_classes)
        self.att_vis_dec = AttModel_vis_dec(glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate, num_classes)

        self.att_syb_enc = AttModel_syb_enc(glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate, num_classes)
        self.att_syb_dec = AttModel_syb_dec(glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate, num_classes)

        self.MIL_align = AttModel_MIL_align(hidden_size, 3, num_heads, dropout_rate, maxlen)
    ...
    def forward(self, vis_fea, vis_mask, q_ipt, q_mask, q_graph, syb_ipt, syb_mask, syb_graph, vis_graph, decMask=True, visGraph=True, mcb=False):
        q_fea = q_ipt

        fea_vis, dec_mask_vis, decoder_inputs_vis, valid_mask_vis = self.att_vis_enc(vis_fea, vis_mask, vis_graph, q_fea, q_graph, q_mask, decMask, visGraph)
        fea_syb, dec_mask_syb, decoder_inputs_syb, valid_mask_syb = self.att_syb_enc(syb_ipt, syb_mask, syb_graph, q_fea, q_graph, q_mask, decMask)

        # print ('valid_mask_vis', valid_mask_vis.size())
        # print ('fea_vis', fea_vis.size())
        fea_vis_align, att_weights = self.MIL_align(fea_vis, valid_mask_vis, fea_syb, valid_mask_syb)
        fea_syb_align = fea_syb.clone()

        dec_vis_align = self.att_vis_dec(fea_vis_align, dec_mask_vis, decoder_inputs_vis)
        dec_syb_align = self.att_vis_dec(fea_syb_align, dec_mask_syb, decoder_inputs_syb)

        logits_vis = self.cls_vis(dec_vis_align).squeeze(1)
        logits_syb = self.cls_syb(dec_syb_align).squeeze(1)

        if mcb == True:
            fea = self.mcb(dec_vis_align, dec_syb_align)
            fea = self.mcb_dropout(fea)
            logits_concat = self.cls_mcb(fea).squeeze(1)
            # print ('logits_concat', logits_concat.size()) #torch.Size([32, 914])
            # print ('logits_vis', logits_vis.size())
        else:
            fea = torch.cat((dec_vis_align.squeeze(), dec_syb_align.squeeze()), 1)
            logits_concat = self.cls(fea)  # 64, 914

        return logits_concat, logits_vis, logits_syb, att_weights
