...
#         self.enc_emb = embedding(self.enc_voc, self.hp.hidden_units, scale=True)
        new_glove_voc = Parameter(torch.rand(400359, 300)) #bua 400189 vqa 400359
        ...
        for i in range(self.hp.num_blocks):
            self.__setattr__('enc_self_attention_%d' % i, new_multihead_attention(num_units=self.hp.hidden_units,
                                                                              ...
                                                                              dropout_rate=0,#self.hp.dropout_rate,
                                                                              causality=False))
    ...
    def forward(self, x, y, graph1, graph2, graph3, x0):
        # define decoder inputs
        #self.decoder_inputs = torch.cat([Variable(torch.ones(y[:, :1].size()).cuda() * 2).long(), y[:, :-1]], dim=-1)  # 2:<S>
        #self.decoder_inputs = torch.cat([Variable(torch.ones(y[:, :1].size()).cuda() * 2).long(), Variable(torch.zeros(y[:, :-1].size()).cuda()).long()], dim=-1)
        ...
        # Blocks
        for i in range(self.hp.num_blocks):
            if i < self.hp.num_blocks//3:
                self.enc = self.__getattr__('enc_self_attention_%d' % i)(self.enc, self.enc, self.enc, graph3)
            elif i < 2*self.hp.num_blocks // 3 and i >= self.hp.num_blocks // 3:
                self.enc = self.__getattr__('enc_self_attention_%d' % i)(self.enc, self.enc, self.enc, graph2)
            else:
...
class AttModel_syb(nn.Module):
    ...
    def forward(self, x, y, graph1, graph2, graph3):
        # define decoder inputs
        #self.decoder_inputs = torch.cat([Variable(torch.ones(y[:, :1].size()).cuda() * 2).long(), y[:, :-1]], dim=-1)  # 2:<S>
        #self.decoder_inputs = torch.cat([Variable(torch.ones(y[:, :1].size()).cuda() * 2).long(), Variable(torch.zeros(y[:, :-1].size()).cuda()).long()], dim=-1)  # 2:<S>
        self.decoder_inputs = Variable(torch.ones(y[:, :1].size()).cuda() * 2).long()
...
class AttModel(nn.Module):
    ...
    def forward(self, x_syb, x_vis, y, graph1_syb1, graph1_vis1, graph1_syb2, graph1_vis2, graph1_syb3, graph1_vis3, x0):
        #x_syb_batch, x_vis_batch, y_batch, g_syb_batch, g_vis_batch, x0_batch
        self.dec_syb = self.att_syb(x_syb, y, graph1_syb1, graph1_syb2, graph1_syb3)
        self.dec_vis = self.att_vis(x_vis, y, graph1_vis1, graph1_vis2, graph1_vis3, x0)
