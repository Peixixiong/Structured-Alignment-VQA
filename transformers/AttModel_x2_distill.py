...
class AttModel_vis_grid(nn.Module):
    ...
    def forward(self, vis_fea, q_fea, q_graph, q_mask):
        ...
        graph_diag[:,vis_fea.shape[1]:, vis_fea.shape[1]:] = q_mask.unsqueeze(2)
        ...
        graph_cross[:,0:vis_fea.shape[1], vis_fea.shape[1]:] = 1
...
class AttModel_syb(nn.Module):
    ...
    def forward(self, syb_ipt, syb_mask, syb_graph, q_fea, q_graph, q_mask):
        ...
        graph_diag[:,0:syb_graph.size(2), 0:syb_graph.size(2)] = syb_mask.unsqueeze(2)
        graph_diag[:,syb_graph.size(2):, syb_graph.size(2):] = q_mask.unsqueeze(2)
        ...
        graph_cross[:,0:syb_graph.size(1), syb_graph.size(1):] = 1
        graph_cross[:,syb_graph.size(1):, 0:syb_graph.size(1)] = 1
        for i in range(2, 4):
            fea_att = self.__getattr__('enc_self_attention_%d' % i)(fea_att, fea_att, fea_att, mask, graph_cross)
            fea_att = self.__getattr__('enc_feed_forward_%d' % i)(fea_att)
