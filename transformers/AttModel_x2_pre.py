...
class AttModel_vis_grid(nn.Module):
    def __init__(self, hidden_size, num_classes, maxlen_q, num_blocks, num_heads, dropout_rate):
        ...
        #self.dec_emb = embedding(1, self.hidden_size, scale=True)
        #self.dec_positional_encoding = nn.Sequential(embedding(1, self.hidden_size, zeros_pad=False, scale=False),
        #                                    nn.Dropout(self.dropout_rate))
    ...
    def forward(self, vis_fea, q_fea, q_graph, q_mask):
        ...
        graph_diag[:,0:vis_fea.shape[1], 0:vis_fea.shape[1]] = 1
        graph_diag[:,vis_fea.shape[1]:, vis_fea.shape[1]:] = q_mask.unsqueeze(1)
        
        # only consider each individual one.
        for i in range(2):
            fea_att = self.__getattr__('enc_self_attention_%d' % i)(fea_att, fea_att, fea_att, mask, graph_diag)
        ...
        graph_cross[:,vis_fea.shape[1]:, 0:vis_fea.shape[1]] = 1
        for i in range(2, 4):
        ...
        #decoder_inputs = torch.zeros((batch_size,1)).cuda().long()
...
class AttModel_syb(nn.Module):
    def __init__(self, glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate):
        ...
        new_glove_voc = Parameter(torch.rand(402000, 300))
    ...
    def forward(self, syb_ipt, syb_mask, syb_graph, q_fea, q_graph, q_mask):
        ...
        graph_diag[:,0:syb_graph.size(2), 0:syb_graph.size(2)] = syb_mask.unsqueeze(1)
        graph_diag[:,syb_graph.size(2):, syb_graph.size(2):] = q_mask.unsqueeze(1)
