...
class AttModel_MIL_align(nn.Module):
    ...
    def forward(self, vis_fea, vis_mask, syb_fea):
        ...
        for i in range(self.num_blocks):
            ...
            att_weights[i] = att_w
        return vis_fea, [att_weights[self.num_blocks-1]]
...
class AttModel(nn.Module):
    def __init__(self, glove, hidden_size, num_classes, maxlen_q, maxlen, maxlen_v, num_blocks, num_heads, dropout_rate,
        ...
        self.att_syb = AttModel_syb(glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate,
                                    num_classes)
        self.MIL_align = AttModel_MIL_align(2048, 3, num_heads, dropout_rate, maxlen)
        ...
        self.syb_emb = nn.Embedding.from_pretrained(new_glove_voc, freeze=False)
        self.syb_mlp0 = nn.Linear(300, 2048)
        self.syb_mlp1 = nn.Linear(2048, hidden_size)


    def forward(self, vis_fea, vis_mask, q_ipt, q_mask, q_graph, syb_ipt, syb_mask, syb_graph, vis_graph, decMask=True,
                visGraph=True, mcb=False):
        # q_fea = self.q_emb(q_ipt)
        q_fea = q_ipt

        t1 = datetime.datetime.now()
        fea_c = self.syb_emb(syb_ipt)
        fea_syb = self.syb_mlp0(fea_c)
        # fea_syb, _ = torch.split(fea0, [syb_ipt.shape[1], q_fea.shape[1]], dim=1)

        #Decoder_mask
        vis_valid_mask = torch.zeros((vis_fea.shape[0], vis_fea.shape[1], 1)).cuda()
        for id in range(vis_mask.size(0)):
            non_zero_mask_id = torch.sum(vis_mask[id], dim=1)
            non_zero_mask_id[non_zero_mask_id.nonzero()] = 1
            vis_valid_mask[id, :, 0] = non_zero_mask_id

        fea_vis_align, att_weights = self.MIL_align(vis_fea, vis_valid_mask, fea_syb)
        fea_vis_grid = self.att_vis_grid(fea_vis_align, vis_mask, vis_graph, q_fea, q_graph, q_mask, decMask, visGraph)
