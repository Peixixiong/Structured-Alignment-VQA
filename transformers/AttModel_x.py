...
class AttModel_vis_grid(nn.Module):
    def __init__(self, hidden_size, num_classes, maxlen_q, num_blocks, num_heads, dropout_rate):
        ...
        self.v_positional_encoding = nn.Sequential(embedding(7 * 7, self.hidden_size, zeros_pad=False, scale=False),
        ...
        for i in range(self.num_blocks // 2):
            self.__setattr__('enc_self_attention_graph_%d' % i, new_multihead_attention_with_graph_mask(num_units=self.hidden_size,
                                                                              num_heads=self.num_heads,
                                                                              dropout_rate=0,#self.dropout_rate,
                                                                              causality=False))
            self.__setattr__('enc_feed_forward_graph_%d' % i, feedforward(self.hidden_size,
                                                                    [4 * self.hidden_size,
                                                                     self.hidden_size]))
    ...
    def forward(self, vis_fea, q_fea, q_graph, q_mask):
        ...
        graph = torch.zeros((vis_fea.shape[0], size_graph, size_graph)).cuda()
        graph[:,vis_fea.shape[1]:, vis_fea.shape[1]:] = q_graph
        for i in range(self.num_blocks // 2):
            fea_graph = self.__getattr__('enc_self_attention_graph_%d' % i)(fea_graph, fea_graph, fea_graph, mask, graph)
            fea_graph = self.__getattr__('enc_feed_forward_graph_%d' % i)(fea_graph)

        fea_graph = fea_graph * mask.unsqueeze(2)
        fea, _ = torch.max(fea_graph, dim = 1)
        return fea
...
class AttModel_syb(nn.Module):
    def __init__(self, glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate):
        ...
        for i in range(self.num_blocks // 2):
            self.__setattr__('enc_self_attention_graph_%d' % i, new_multihead_attention_with_graph_mask(num_units=hidden_size,
                                                                              num_heads=self.num_heads,
                                                                              dropout_rate=0,#dropout_rate,
                                                                              causality=False))
            self.__setattr__('enc_feed_forward_graph_%d' % i, feedforward(hidden_size,
                                                                    [4 * hidden_size,
                                                                     hidden_size]))
    ...
    def forward(self, syb_ipt, syb_mask, syb_graph, q_fea, q_graph, q_mask):
        ...
        graph = torch.zeros((batch_size, size_graph, size_graph)).cuda()
        ...
        fea, _ = torch.max(fea, dim = 1) # equals ReLU and maxpooling
        return fea

class AttModel(nn.Module):
    def __init__(self, glove, hidden_size, num_classes, maxlen_q, maxlen, num_blocks, num_heads, dropout_rate ):
        super(AttModel, self).__init__()
        self.att_vis_grid = AttModel_vis_grid(hidden_size, num_classes, maxlen_q, num_blocks, num_heads, dropout_rate)
    ...
    def forward(self, vis_fea, q_ipt, q_mask, q_graph, syb_ipt, syb_mask, syb_graph):

        q_fea = self.q_emb(q_ipt)

        fea_syb = self.att_syb(syb_ipt, syb_mask, syb_graph, q_fea, q_graph, q_mask)
        fea_vis_grid = self.att_vis_grid(vis_fea, q_fea, q_graph, q_mask)
        
        fea = torch.cat((fea_syb, fea_vis_grid), 1)
