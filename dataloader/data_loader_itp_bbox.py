...
class GQADataset_topN(data.Dataset):
    ...
    def convert_graph(self, data_info, bg_class, bbox):
        ...
        for row_idx, (obj_idxs, attr_idx) in enumerate(zip(data_info['objects_id'], data_info['attrs_id'])):
            nodes_obj = []
        ...
        try:
            ...
            with tarfile.open(self.g_tar_fn) as tar_fid:
                ...
                if self.opt.model_v != 23:
                    nodes, edges, vis_edges, keep_bbox, idx_of_obj, vis_nodes, syb2vis, valid2all  = self.convert_graph(data_info, self.opt.bg_class, bbox)
                else:
...
class GQADataset(data.Dataset):
    def __init__(self, split, opt, fea_tar_fn, q_tar_fn, g_tar_fn, with_loc = True, with_gt_relation = False):
...
class GQADataset_aug(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with tarfile.open(self.g_tar_fn) as tar_fid:
            ...
            data_info = data['info'].tolist()
            if self.opt.model_v != 23:
                nodes, edges, vis_edges, keep_bbox, idx_of_obj, vis_nodes, syb2vis, valid2all = self.convert_graph(data_info,
                                                                                                        self.opt.bg_class,
                                                                                                        bbox, self.opt.aug_rate)
            else:
                nodes, edges, vis_edges, keep_bbox, idx_of_obj, vis_nodes, syb2vis, valid2all = self.convert_graph_visrel(
