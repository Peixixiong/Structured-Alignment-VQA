...
class GQADataset(data.Dataset):
    def __init__(self, split, opt, fea_tar_fn, q_tar_fn, g_tar_fn, with_loc = False, with_gt_relation = False):
    ...
    def convert_graph_visrel(self, data_info, bg_class, bbox):
        ...
        return nodes, relation, vis_relation, keep_idx, idx_obj, vis_nodes, syb2vis
        ...
        for i in range(num_obj):
            for j in range(num_obj):
                ...
                if key in self.gt_relations:
                    ...
                    relation.append([idx_obj[i], pos_rel])
                    relation.append([pos_rel, idx_obj[j]])
                    vis_relation.append([valid2all[syb2vis[idx_obj[i]]], valid2all[syb2vis[idx_obj[j]]]])

        #Make Background Visual Feature Connect to every other Visual Feature.
        total_len = len(keep_idx)
        for id, item in enumerate(keep_idx):
            if item == 0:
                for i in range(total_len):
                    vis_relation.append([id, i])
                    vis_relation.append([i, id])

        return nodes, relation, vis_relation, keep_idx, idx_obj, [None], [-1]
    ...
    def __getitem__(self, index):
        ...
        with tarfile.open(self.g_tar_fn) as tar_fid:
            ...
            if self.opt.model_v != 23:
                nodes, edges, vis_edges, keep_bbox, idx_of_obj, vis_nodes, syb2vis  = self.convert_graph(data_info, self.opt.bg_class, bbox)
            else:
                nodes, edges, vis_edges, keep_bbox, idx_of_obj, vis_nodes, syb2vis = self.convert_graph_visrel(data_info, self.opt.bg_class, bbox)
        ...
        return vis_fea, np.asarray(nodes_idx).astype('int64'), edges, vis_edges, bbox, np.asarray(qnode_idx).astype('int64'), \
               qedge, answer, idx_of_obj, np.asarray(vis_nodes_idx).astype('int64'), np.asarray(syb2vis).astype('int64'),
    
    def __len__(self):
        return len(self.q_list)


class GQADataset_aug(data.Dataset):
    def __init__(self, split, opt, fea_tar_fn, q_tar_fn, g_tar_fn, with_loc=False, with_gt_relation=False):
        super(GQADataset_aug, self).__init__()
        self.split = split
        self.opt = opt
        self.with_loc = with_loc
        self.pos_grid_num = 10
    ...
    def convert_graph_visrel(self, data_info, bg_class, bbox, aug_rate):
        weights = [1 - aug_rate, aug_rate]
        population = [0, 1]
        ...
        for row_idx, (obj_idx, attr_idx) in enumerate(zip(data_info['objects_id'], data_info['attrs_id'])):
            if self.split == 'train':
                if choices(population, weights) == 1:
                    obj_idx = random.randint(0, len(self.vg_classes) - 1)
                if choices(population, weights) == 1:
                    attr_idx = random.randint(0, len(self.vg_attrs)-1)
            if obj_idx >= len(self.vg_classes):
                vis_nodes_obj.append('__background__')
                vis_nodes_attr.append('__background_attr__')
                continue
    ...
    def convert_graph(self, data_info, bg_class, bbox, aug_rate):
        weights = [1 - aug_rate, aug_rate]
        population = [0, 1]
    ...
    def __getitem__(self, index):
        ...
        with tarfile.open(self.g_tar_fn) as tar_fid:
            ...
            data_info = data['info'].tolist()
            if self.opt.model_v != 23:
                nodes, edges, vis_edges, keep_bbox, idx_of_obj, vis_nodes, syb2vis = self.convert_graph(data_info,
                                                                                                        self.opt.bg_class,
                                                                                                        bbox, self.opt.aug_rate)
            else:
                nodes, edges, vis_edges, keep_bbox, idx_of_obj, vis_nodes, syb2vis = self.convert_graph_visrel(
                    data_info, self.opt.bg_class, bbox, self.opt.aug_rate)
            idx_of_obj = np.asarray(idx_of_obj).astype('int64')
...
def collate_fn_vis(data):
    data = list(filter(lambda x: x is not None, data))
    vis_fea, nodes_idx, edges, vis_edges, bbox, qnode_idx, qedge, answer, idx_of_obj, vis_nodes_idx, syb2vis = zip(*data)
    ...
    # vis_rel_att branch
    max_vis_node_len = 0
    for node in vis_nodes_idx:
        max_vis_node_len = max(max_vis_node_len, node.shape[0])
    ...
    vis_syb_node = np.zeros((batch_size, max_vis_node_len), dtype='int64')
    fea_ipt_mask = np.zeros((batch_size, max_vis_node_len, max_vis_node_len), dtype='int32') #new vis_syb mask
    vis_w_rel = np.zeros((batch_size, max_vis_node_len, max_vis_node_len), dtype='int32') #new vis_syb mask
    node_ipt_vis_graph = np.zeros((batch_size, max_vis_node_len, max_vis_node_len), dtype='int32')
    vis_syb_node[:] = PAD
    vis_w_rel[:] = VIS_PAD

    # for i in range(batch_size):
    #     fea_ipt[i, 0:vis_fea[i].shape[0], :] = vis_fea[i]
    #     vis_syb_node[i, 0:vis_nodes_idx[i].shape[0]] = vis_nodes_idx[i]
    #     fea_ipt_mask[i, 0:vis_nodes_idx[i].shape[0], 0:vis_nodes_idx[i].shape[0]] = 1
    #

    # generate vis graph and syb2vis link graph
    # for row, (edge, syb2vis_link) in enumerate(zip(vis_edges, syb2vis)):
    ...
    #
    #     syb2vis_link = np.asarray(syb2vis_link).astype('int32')
    #     if syb2vis_link.size == 0:
    #         continue
    #     if len(syb2vis_link.shape) == 1:
    #         syb2vis_link = syb2vis_link[np.newaxis, :]
    #     vis_w_rel[row, syb2vis_link[:, 0], syb2vis_link[:, 1]] = 1
    ...
    data_ipt['vis_syb'] = torch.from_numpy(vis_syb_node)
    data_ipt['vis_syb_link'] = torch.from_numpy(vis_w_rel)
    data_ipt['vis_fea_mask'] = torch.from_numpy(fea_ipt_mask) #vis_mask new vis_syb mask
