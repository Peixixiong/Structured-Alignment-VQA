...
class AttModel_vis(nn.Module):
    ...
    def forward(self, vis_fea, vis_syb_ipt, vis_syb_mask, vis_syb_link, vis_syb_graph, q_fea, q_graph, q_mask, decMask, visGraph): #vis_syb_mask is syb2vis
        batch_size = vis_syb_ipt.shape[0]
        fea = torch.cat([vis_syb_ipt, q_fea], dim=1)
        fea = self.syb_emb(fea)
        fea = self.syb_mlp(fea)

        #Do not use vis_mask, here vis obj is scattered in inputs, check syb2vis

        # vis_mask_ = torch.sum(vis_mask, dim=-1)
        # vis_mask_one = torch.ones(vis_mask_.size())
        # vis_mask_new = torch.where(vis_mask_ > 0, vis_mask_one, vis_mask_).unsqueeze(2).repeat(1, 1, vis_fea.size(2))
        #
        # vis_mask_exp = torch.zeros(fea.size()).long().cuda() #This is for padding issue.
        # if vis_mask_new.size(1) <= vis_mask_exp.size(1):
        #     vis_mask_exp[:,:vis_mask_new.size(1),:] = vis_mask_new
        # else:
        #     vis_mask_exp = vis_mask_new[:,:vis_mask_exp.size(1),:]
        #
        # fea = vis_mask_exp*vis_fea + (1-vis_mask_exp)*fea
        # fea = self.syb_mlp2(fea)
        for mask_id, vis_syb_ in enumerate(vis_syb_link):
            fea[mask_id, (vis_syb_ + 1).nonzero(), :] = vis_fea[mask_id, :(vis_syb_ + 1).nonzero().size(0), :].unsqueeze(1)
        ...
        size_graph = vis_syb_ipt.shape[1] + q_graph.shape[1]
        mask = torch.zeros((vis_syb_ipt.shape[0], size_graph, size_graph)).cuda()
        graph_diag = torch.zeros((vis_syb_ipt.shape[0], size_graph, size_graph)).cuda()

        # Decoder_mask
        dec_mask = torch.zeros((vis_syb_ipt.shape[0], 1, size_graph)).cuda()

        for id in range(mask.size(0)):
            mask[id] = torch.block_diag(vis_syb_mask[id], q_mask[id])
        ...
        graph = graph_cross
        if visGraph == False:
            graph[:, :vis_syb_ipt.shape[1], :vis_syb_ipt.shape[1]] = 1
        else:
            graph[:, :vis_syb_ipt.shape[1], :vis_syb_ipt.shape[1]] = vis_syb_graph

        graph[:, vis_syb_ipt.shape[1]:, vis_syb_ipt.shape[1]:] = q_graph
        fea_att = fea
...
class AttModel(nn.Module):
    def __init__(self, glove, hidden_size, num_classes, maxlen_q, maxlen, maxlen_v, num_blocks, num_heads, dropout_rate, dropout_rate_mcb ):
        ...
        self.att_vis = AttModel_vis(glove, hidden_size, maxlen, maxlen_q, num_blocks, num_heads, dropout_rate, num_classes)
    ...
    def forward(self, vis_fea, vis_syb_new, vis_new_mask, vis_syb_link, vis_new_graph, q_ipt, q_mask, q_graph, syb_ipt, syb_mask, syb_graph, decMask=True, visGraph=True):
        #vis_fea, vis_syb_ipt, vis_syb_mask, vis_syb_graph, q_ipt, q_mask, q_graph, syb_ipt, syb_mask, syb_graph, decMask=True, visGraph=True
        # q_fea = self.q_emb(q_ipt)
        q_fea = q_ipt

        fea_vis = self.att_vis(vis_fea, vis_syb_new, vis_new_mask, vis_syb_link, vis_new_graph, q_fea, q_graph, q_mask, decMask, visGraph)
        logits_vis = self.cls_vis(fea_vis).squeeze(1)
        fea_syb = self.att_syb(syb_ipt, syb_mask, syb_graph, q_fea, q_graph, q_mask, decMask)
        ...
        fea = torch.cat((fea_syb.squeeze(), fea_vis.squeeze()), 1)
        logits_concat = self.cls(fea) #64, 914

        return logits_concat, logits_vis, logits_syb
