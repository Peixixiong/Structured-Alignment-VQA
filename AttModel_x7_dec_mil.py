...
class AttModel_MIL_align(nn.Module):
    def __init__(self, hidden_size, num_blocks, num_heads, dropout_rate):
        ...
        for i in range(self.num_blocks):
            self.__setattr__('align_enc_self_attention_%d' % i, new_multihead_attention_with_graph_mask(num_units=hidden_size,
                                                                              num_heads=self.num_heads,
                                                                              dropout_rate=dropout_rate,
                                                                              causality=False, return_att = True))
            self.__setattr__('align_enc_feed_forward_%d' % i, feedforward(hidden_size,
                                                                    [4 * hidden_size,
    ...
    def forward(self, vis_fea, vis_mask, syb_fea, syb_mask):
        ...
        for i in range(self.num_blocks):
            fea = torch.cat([ vis_fea, syb_fea], dim = 1)
            fea = self.__getattr__('enc_self_attention_%d' % i)(fea, fea, fea, mask, None)
            fea = self.__getattr__('enc_feed_forward_%d' % i)(fea)
            vis_fea, syb_fea = torch.split(fea, [vis_fea.shape[1], syb_fea.shape[1]], dim = 1)

            vis_fea, att_w = self.__getattr__('align_enc_self_attention_%d' % i)(vis_fea, syb_fea, syb_fea, syb_mask, None)
            vis_fea = self.__getattr__('align_enc_feed_forward_%d' % i)(vis_fea)
            att_weights.append(att_w)

        return vis_fea, syb_fea, att_weights
...
class AttModel(nn.Module):
    def __init__(self, glove, hidden_size, num_classes, maxlen_q, maxlen, maxlen_v, num_blocks, num_heads, dropout_rate ):
        ...
        self.MIL_align = AttModel_MIL_align(hidden_size, 3, num_heads, dropout_rate)
        ...
        self.__setattr__('gq_self_attention',
                             new_multihead_attention_with_graph_mask(num_units=self.hidden_size,
                                                 num_heads=self.num_heads,
                                                 dropout_rate=self.dropout_rate,
                                                 causality=False))
        self.__setattr__('gq_self_feed_forward', feedforward(self.hidden_size,
                                                                    [4 * self.hidden_size,
                                                                     self.hidden_size]))
    ...
    def forward(self, vis_fea, vis_mask, q_ipt, q_mask, q_graph, syb_ipt, syb_mask, syb_graph, idx_of_obj):
        ...
        fea_vis_align, fea_syb_align, att_weights = self.MIL_align(fea_vis, vis_mask, fea_syb, syb_mask)

        fea_vis_align *= vis_mask.unsqueeze(2)
        fea_syb_align *= syb_mask.unsqueeze(2)
        ...
        q_fea = self.__getattr__('gq_self_attention')(q_fea,q_fea,q_fea,q_mask,q_graph)
        q_fea = self.__getattr__('gq_self_feed_forward')(q_fea)

        q_fea *= q_mask.unsqueeze(2)
        
        fea = torch.cat([fea_vis_align, fea_syb_align], dim = 1)

        size_graph = fea_vis_align.shape[1] + fea_syb_align.shape[1]

        mask = torch.cat([vis_mask, syb_mask], dim = 1)
        
       
        for i in range(self.num_blocks):
            q_fea = self.__getattr__('enc_self_attention_%d' % i)(q_fea, fea, fea, mask, None)
            q_fea = self.__getattr__('enc_feed_forward_%d' % i)(q_fea)
        ...
        for i in range(self.num_blocks):
            dec_vis_q = self.__getattr__('dec_vanilla_attention_%d' % i)(dec_vis_q, q_fea, q_fea, q_mask, None)
            dec_vis_q = self.__getattr__('dec_feed_forward_%d' % i)(dec_vis_q)

        logits = self.cls(dec_vis_q.squeeze())
